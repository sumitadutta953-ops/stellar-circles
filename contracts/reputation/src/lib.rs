...
impl ReputationContract {
    ...
    pub fn mint_completion_badge(env: Env, member: Address) {
        ...
        let mut rep: u32 = env.storage().persistent().get(&DataKey::Reputation(member.clone())).unwrap_or(0);
        rep += 1;
        env.storage().persistent().set(&DataKey::Reputation(member), &rep);
    }

    pub fn get_reputation_score(env: Env, member: Address) -> u32 {
        env.storage().persistent().get(&DataKey::Reputation(member)).unwrap_or(0)
    }
}
